...
static EVENT_HANDLER(mobility)
{
    ...
    if(walk->paused) {
        ...
        while((int)newdest.x == (int)walk->now.x && (int)newdest.y == (int)walk->now.y);
...
void init_mobility(double walkspeed_metres_per_sec, int pausetime_secs, int nnodes)
{
